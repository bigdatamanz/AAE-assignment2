function [TckResultWLS, navSolutionsWLS] = tracking_POS_WLS_TASK2(posSV, file,signal,track,cmn, Acquired,TckResult_Eph, cnslxyz,eph,sbf,solu)
%Purpose:
...
% ==== skymask ====
skymask_data = readmatrix('C:\Users\13471\OneDrive\Desktop\AAE6102_Assignment2\Code\skymask_A1_urban.csv');
az_mask = skymask_data(:,1);
el_mask = skymask_data(:,2);
...
% ==== weight adjust with respect to skymask ====
az_now = az;
el_now = el;

visibility = zeros(1, svlength); 
for i = 1:svlength
    az_idx = mod(round(az_now(i)), 360) + 1; 
    if az_idx > 360, az_idx = az_idx - 360; end
    el_thresh = el_full(az_idx); 
    if el_now(i) >= el_thresh
        visibility(i) = 1;  
    else
        visibility(i) = 0; 
    end
...
% 
weight_base = CN0_CT(end, :);
% 
weight_adjusted = weight_base;
weight_adjusted(visibility == 0) = weight_adjusted(visibility == 0) * 0.8; 
%weight_adjusted(visibility == 0) = weight_adjusted(visibility == 0) * 0.5; 
%weight_adjusted(visibility == 0) = weight_adjusted(visibility == 0) * 0.3; 
%weight_adjusted(visibility == 0) = weight_adjusted(visibility == 0) * 0; 

% ========
[estusr_wls, dop] = wlspos(prvec, svxyzr, estusr_wls, 1e-3, el_now, weight_adjusted);
...
close(h);
 ...
 %--- Satellite sky plot -----------------------------------------------
    skyPlot(handles, ...
            ...
            posSV);
        title (handles, ['Sky plot']); 
    % title (handles, ['Sky plot (mean PDOP: ', ...
    %                            num2str(mean(navSolutionsWLS.DOP(2,:))), ')']);  
plot_sky_with_mask_and_satellites(navSolutionsWLS, posSV, 'C:\Users\13471\OneDrive\Desktop\AAE6102_Assignment2\Code\skymask_A1_urban.csv');
...
% 定义不同权重系数
weight_factors = [0.8, 0.5, 0.3, 0];
rmse_results = zeros(length(weight_factors), 4); % 存储ENU三个方向和3D位置的RMSE

% 获取参考位置（ground truth）
% 根据环境选择对应的ground truth
if strcmp(file.fileName, 'urban') % 如果是城市环境
    gt_llh = [22.3198722, 114.209101777778, 3]; % 城市环境ground truth
else % 如果是开阔天空环境
    gt_llh = [22.328444770087565, 114.1713630049711, 3]; % 开阔天空环境ground truth
end

% 将ground truth从经纬度转换为ECEF坐标
gt_xyz = llh2xyz([gt_llh(1)/180*pi, gt_llh(2)/180*pi, gt_llh(3)]);

% 计算不同权重下的RMSE
for w_idx = 1:length(weight_factors)
    % 重新运行定位部分，使用不同的权重因子
    for pos_idx = 1:posIndex
        % 重新加载需要的数据
        prvec_now = navSolutionsWLS.prvec(pos_idx,:);
        svxyzr_now = navSolutionsWLS.svxyzr(:,:,pos_idx);
        el_now = navSolutionsWLS.satEA(pos_idx,:);
        az_now = navSolutionsWLS.satAZ(pos_idx,:);
        ...
        weight_base = CN0_CT(end, :);
        weight_adjusted = weight_base;
        weight_adjusted(visibility == 0) = weight_adjusted(visibility == 0) * weight_factors(w_idx);
        
        % 使用新权重重新计算位置
        [estusr_wls_new, ~] = wlspos(prvec_now, svxyzr_now, estusr_wls, 1e-3, el_now, weight_adjusted);
        
        % 保存新的ENU位置
        temp_enu = xyz2enu(estusr_wls_new(1:3), gt_xyz);
        navSolutionsWLS.temp_enu(pos_idx,:,w_idx) = temp_enu;
    end
    
    % 计算RMSE
    errors = navSolutionsWLS.temp_enu(:,:,w_idx);
    
    % 计算ENU三个方向的RMSE
    for i = 1:3 % E, N, U方向
        rmse_results(w_idx, i) = sqrt(mean(errors(:,i).^2));
    end
    
    % 计算3D位置RMSE
    rmse_results(w_idx, 4) = sqrt(mean(sum(errors.^2, 2)));
end

% 输出结果
fprintf('\n===== RMSE Analysis Results =====\n');
fprintf('Weight Factor\tEast(E)\t\tNorth(N)\t\tUp(U)\t\t3D Position\n');
for w_idx = 1:length(weight_factors)
    fprintf('%.1f\t\t%.4f\t%.4f\t%.4f\t%.4f\n', weight_factors(w_idx), ...
        rmse_results(w_idx, 1), rmse_results(w_idx, 2), rmse_results(w_idx, 3), rmse_results(w_idx, 4));
end

% Calculate mean RMSE
mean_rmse = mean(rmse_results, 1);
fprintf('\n===== Mean RMSE Results =====\n');
fprintf('Mean East(E)\tMean North(N)\tMean Up(U)\tMean 3D Position\n');
fprintf('%.4f\t\t%.4f\t\t%.4f\t\t%.4f\n', mean_rmse(1), mean_rmse(2), mean_rmse(3), mean_rmse(4));

% Plot comparison
figure;
bar(rmse_results);
set(gca, 'XTickLabel', {'0.8', '0.5', '0.3', '0'});
legend('East Error', 'North Error', 'Up Error', '3D Position Error');
xlabel('Weight Adjustment Factor for Invisible Satellites');
ylabel('RMSE (meters)');
title('RMSE Comparison for Different Weight Adjustment Strategies');
grid on;

% Save RMSE results
save(['rmse_results_', file.fileName], 'rmse_results', 'weight_factors', 'mean_rmse');

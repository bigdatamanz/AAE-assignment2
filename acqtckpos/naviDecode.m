...
%	ALLTckResult  	- conventional scalar tracking results
%Outputs:
%	ephemeris       - ephemeris obtained from the navigation bit stream
%	ALLTckResult	- revised conventional scalar tracking results
...
%%
% Setting the paramethers of C/N0 estimation (PRM, Power Ratio Method)
M = 20; K =1; T = 1e-3;
...
startOffset = 2000; % avoid noise due to tracking loop transition, 28/04/2020, BING XU
...
for svindex = 1 : length(Acquired.sv)
    ...
    fprintf('\n');
    RawNavigationData = ALLTckResult(prn).P_i(1+startOffset:end); % avoid noise due to tracking loop transition, 28/04/2020, BING XU
    
    % To avoid outliers due to noise, 28/04/2020, BING XU
    for i = 2:length(RawNavigationData)-1
        if (sign(RawNavigationData(i-1)) == sign(RawNavigationData(i+1))) && ...
                (sign(RawNavigationData(i)) ~= sign(RawNavigationData(i-1)))
            RawNavigationData(i) = -RawNavigationData(i);
        end
    end
    ...
    startOffset_2 = startms + startOffset;
    RawNavigationData = ALLTckResult(prn).P_i(1+startOffset_2-1:end);
    NaviDatams(find(RawNavigationData>=0))      = 1;
    NaviDatams(find(RawNavigationData<0))       = -1;
    NaviDataXORms(find(RawNavigationData>=0))   = 0;
    NaviDataXORms(find(RawNavigationData<0))    = 1;
    ...
    %     ALLTckResult(prn).nav1  = startms + startOffset_2 -1;
    for_prest.nav1(prn) = startms + startOffset_2 - 1;
    ...
    %     for index = startms + startOffset_2 -1 : msdatalength-1 % 30/04/2020, BING XU
    %         idx = idx + 1;
    %         index_k = mod(ceil(idx / M),K);
    %         if (index_k)==0
    %             index_k = K;
    %         end
    ...
    navidatalength  = length(NaviDataXOR); % 20 ms
    ...
    for index = 8 : navidatalength
        ...
        if length(index : navidatalength) > 360 % 300 next subframe
            if abs(sum((NaviData(index-7:index).*preamble)))>7.99 && ...
                    abs(sum((NaviData(index-7+300:index+300).*preamble)))>7.99 % find preamble
                ...
                if end_HOW ~= 0 && end_HOW2~=0
                    ...
                    if flag == 1
                        ...
                        for idx = 1:num_sf
                            ...
                            ephemeris(prn).sfb  = [ephemeris(prn).sfb (index-7)+(idx-1)*300];% index (in unit of 20ms) of each subframe, 30/04/2020
                            ephemeris(prn).TOW  = [ephemeris(prn).TOW (bin2dec_GPSSDR(subframe(idx,47:-1:31))-1)*6]; % -1 because of z-count2TOW transformation % Modified on 1 Apr 2021 1024
                            ...
                            switch subframeid
                                case 1
                                    ...
                                    ephemeris(prn).weeknum      = [ephemeris(prn).weeknum bin2dec_GPSSDR(subframe(idx,70:-1:61)) + 1024+1024];%% !!
                            ...
                            if flag_sf(svindex,1)==1 & flag_sf(svindex,2)==1 & flag_sf(svindex,3)==1 & flag_sf(svindex,4)==1 & flag_sf(svindex,5)==1 ...
                                ...
                                ephemeris(prn).updatetime_tow = [ephemeris(prn).updatetime_tow, ephemeris(prn).TOW(end) + 6] ; % unit: second (+6 because of its next frame)

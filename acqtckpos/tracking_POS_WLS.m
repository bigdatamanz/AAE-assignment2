function [TckResultWLS, navSolutionsWLS] = tracking_POS_WLS(posSV, file,signal,track,cmn, Acquired,TckResult_Eph, cnslxyz,eph,sbf,solu)
%Purpose:
...
az_full = 0:359; % 每度一个
el_full = interp1(az_mask, el_mask, az_full, 'linear'); % 插值得到每个方位角的遮挡仰角
...
% weight_adjusted(visibility == 0) = weight_adjusted(visibility == 0) * 0.8; % 遮挡卫星权重降低10倍
% 
% % ==== 最小二乘带入调整后的权重 ====

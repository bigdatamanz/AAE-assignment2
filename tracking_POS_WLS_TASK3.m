...
counter_corr    = corrUpt-1 * ones(svlength,1);
    ...
    if sum(flg_pos) == svlength
        ...
        while true
            ...
            % CN0_CT同步删整列
            weight_wls(bad_idx) = []; % 不只是最后一行
delayValue(bad_idx,:) = [];
            % 更新svlength
            svlength = length(sv);
            flg_pos = zeros(1, svlength); % 🔥🔥 重新同步更新
% 根据新的svlength重新初始化这些数组
...
% delayValue本质是 [svlength, datalength/pdi]的数组，也需要按行删对应bad_idx！
...
PL = K_md * sqrt(lambda_max) * sigma;
        ...
        %%
        navSolutionsWLS.svxyzr{posIndex}     = svxyzr;           % 改大括号
        navSolutionsWLS.codePhaseMeas{posIndex} = codePhaseMeas; % 改大括号
...
for i = 1:num_epochs
    ...
    position_error(i) = norm(true_pos - est_pos); % 欧氏距离
end
